...
- **Retry classification:** `retry.ts` must select the `x-death` entry by *queue name + `reason: rejected`*, not index 0. Non-retryable error classes and `MAX_ATTEMPTS` must both route to `q.parked`, and the Redis record must be marked `failed` with a reason.
- **Prefetch:** `prefetch(1, true)` — the `global` flag is load-bearing. Without it the three consumers each get their own slot and one container runs three jobs at once.
