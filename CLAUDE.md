...
- **Layering:** route (HTTP only) → `*-service` (orchestration) → `*-repository` (the single access path to one datastore) → client. Repositories and services export their type as `ReturnType<typeof createX>` rather than a hand-written interface; only *input* contracts are hand-written. Deps narrow with `Pick<>` to the methods actually called.
